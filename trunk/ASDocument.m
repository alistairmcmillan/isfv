...
- (void)makeWindowControllers
{
	...
	[windowController populateData:_data];
}
...
- (void)parseSFV:(NSString *)fileContents {
	...
	[_data setFiles:files withCheckSums:checkSums];
}

- (NSString*)createSFV {
	NSMutableString* result = [[NSMutableString alloc] init];
	NSString* date = [[NSDate date] description];
	if([[ASPreferenceController objectForKey:WinSFVFormat] boolValue]) {
		[result appendFormat:@"; Generated by WIN-SFV32 v1.1a on %@\r\n;\r\n", date];
	} else {
		NSString* sig = [[[NSBundle mainBundle] infoDictionary] objectForKey:
			@"CFBundleSignature"];
		NSString* ver = [[[NSBundle mainBundle] infoDictionary] objectForKey:
			@"CFBundleVersion"];
		[result appendFormat:@"; Generated by %@ %@ on %@\r\n;\r\n", sig, ver, date];
	}
	if([[ASPreferenceController objectForKey:DatesTimes] boolValue]) {
		if ([self fileURL]) {
			NSString* path = [[[[self fileURL] relativePath]
...
- (NSData *)dataOfType:(NSString *)typeName error:(NSError **)outError {
    ...
    return [[self createSFV] dataUsingEncoding:NSUTF8StringEncoding];
}
